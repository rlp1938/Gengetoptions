...
{	/* fread() with error handling, fn is for error reporting only. */
	size_t wasread = fread(fro, 1, nbytes, fpi);
	if (wasread != nbytes && ferror(fpi)) {
...
{	/* WTF! when relpath begins with '~' realpath() bombs! */
	static char rpath[PATH_MAX];
	char *res;
	if (relpath[0] == '~') {
		char buf[PATH_MAX];
		strcpy(buf, getenv("HOME"));
		strcat(buf, relpath + 1);
		res = realpath(buf, rpath);
	} else {
		res = realpath(relpath, rpath);
	}
	if (!res) {	// failed
		perror(relpath);
		exit(EXIT_FAILURE);
...
{	/* will abort with error message on error, no return value needed */
	const off_t chunk = 128 * 1024 * 1024;	// 128 megs
	struct stat sb;
	if (stat(filefro, &sb) == -1) {
		perror(filefro);
		exit(EXIT_FAILURE);
	}
	if (sb.st_size <= chunk) {
		fdata dat = readfile(filefro, 0, 1);
		writefile(fileto, dat.from, dat.to, "w");
		free(dat.from);
	} else {
		copyfilebychunk(chunk, filefro, fileto);
	}
} // copyfile()

void copyfilebychunk(const size_t chunk, const char *filefro,
						const char *fileto)
{	/* copy filefro to fileto 1 chunk at a time */
	char *buf = docalloc(1, chunk, "copyfilebychunk()");
	FILE *fpi, *fpo;
	fpi = dofopen(filefro, "r");
	fpo = dofopen(fileto, "w");
	while (1) {
		size_t bytesread = dofread(filefro, buf, chunk, fpi);
		dofwrite(fileto, buf, bytesread, fpo);
		if (bytesread != chunk) break;	// errors handled by read/write
	}
	free(buf);
	dofclose(fpo);
	dofclose(fpi);
...
{	/* copies ownership, perms and times from src to dst.
	   In error writes particulars to 'copy_errors.lst' in the current
	*  dir and continues. */
	struct stat sb;
	lstat(src, &sb); // the file to stat has been checked long before.
	int result = lchown(dst, sb.st_uid, sb.st_gid);
	if (result == -1)
		error2file("copy_errors.lst", "chown", errno);
	result = chmod(dst, sb.st_mode);
	if (result == -1)
		error2file("copy_errors.lst", "chmod", errno);
	struct utimbuf utb;
	utb.actime = sb.st_atime;
	utb.modtime = sb.st_mtime;
	result = utime(dst, &utb);
	if (result == -1 )
		error2file("copy_errors.lst", "times", errno);
} // copyfileattributes()

void error2file(const char *efile, const char *text, int num)
{
	int errsv = num;
	FILE *fpo = dofopen(efile, "a");
